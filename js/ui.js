...
const UI = {
    ...
    renderProductsTab(products) {
        ...
        const summaryContainer = document.getElementById('productsSummaryCards');

        if (products.length === 0) {
            container.innerHTML = '<p class="empty-state">No products yet. Click "+ Add Product" to begin.</p>';
            summaryContainer.innerHTML = '';
            return;
        }

        // Calculate summary stats (only active products)
        let totalProducts = products.length;
        let activeProducts = products.filter(p => !p.is_discontinued);
        let avgMargin = 0;
        activeProducts.forEach(p => {
            const profit = p.selling_price - p.cost_of_goods;
            const margin = p.selling_price > 0 ? (profit / p.selling_price) * 100 : 0;
            avgMargin += margin;
        });
        avgMargin = activeProducts.length > 0 ? avgMargin / activeProducts.length : 0;
        const discontinuedCount = totalProducts - activeProducts.length;

        summaryContainer.innerHTML = `
            <div class="products-summary-card">
                <span class="products-summary-label">Active Products</span>
                <span class="products-summary-value">${activeProducts.length}</span>
            </div>
            <div class="products-summary-card">
                <span class="products-summary-label">Avg. Gross Margin</span>
                <span class="products-summary-value">${avgMargin.toFixed(1)}%</span>
            </div>
            ${discontinuedCount > 0 ? `<div class="products-summary-card">
                <span class="products-summary-label">Discontinued</span>
                <span class="products-summary-value">${discontinuedCount}</span>
            </div>` : ''}
        `;
        ...
        // Sort: active products first, then discontinued
        const sorted = [...products].sort((a, b) => (a.is_discontinued || 0) - (b.is_discontinued || 0));

        sorted.forEach(p => {
            const priceAfterTax = p.selling_price * (1 + (p.tax_rate || 0) / 100);
            const profit = p.selling_price - p.cost_of_goods;
            const margin = p.selling_price > 0 ? (profit / p.selling_price) * 100 : 0;
            const marginClass = margin >= 50 ? 'margin-high' : margin >= 20 ? 'margin-mid' : 'margin-low';
            const discontinued = p.is_discontinued ? ' product-discontinued' : '';
            const toggleTitle = p.is_discontinued ? 'Reactivate' : 'Discontinue';
            const toggleIcon = p.is_discontinued ? '&#8635;' : '&#8856;';

            html += `<tr class="${discontinued}">
                <td class="product-name-cell">
                    <span class="product-name">${Utils.escapeHtml(p.name)}</span>
                    ${p.is_discontinued ? '<span class="product-discontinued-badge">Discontinued</span>' : ''}
                    ${p.notes ? `<span class="product-notes-hint" title="${Utils.escapeHtml(p.notes)}">&#9998;</span>` : ''}
                </td>
                <td class="product-sku">${p.sku ? Utils.escapeHtml(p.sku) : '<span class="text-muted">&mdash;</span>'}</td>
                <td class="amount-receivable">${Utils.formatCurrency(p.selling_price)}</td>
                <td>${(p.tax_rate || 0).toFixed(2)}%</td>
                <td>${Utils.formatCurrency(priceAfterTax)}</td>
                <td class="amount-payable">${Utils.formatCurrency(p.cost_of_goods)}</td>
                <td class="${profit >= 0 ? 'amount-receivable' : 'amount-payable'}">${Utils.formatCurrency(profit)}</td>
                <td><span class="product-margin-badge ${marginClass}">${margin.toFixed(1)}%</span></td>
                <td class="product-actions-cell">
                    <button class="btn-icon toggle-product-btn" data-id="${p.id}" title="${toggleTitle}">${toggleIcon}</button>
                    <button class="btn-icon edit-product-btn" data-id="${p.id}" title="Edit">&#9998;</button>
                    <button class="btn-icon delete-product-btn" data-id="${p.id}" title="Delete">&times;</button>
                </td>
            </tr>`;
        });
