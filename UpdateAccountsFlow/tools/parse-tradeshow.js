...
/**
 * Parse Trade Show POS Excel file.
 *
 * Known columns:
 * Transaction no | Date | Sales representative | Billing name | Company |
 * Address | City | State/province | Zip code/postcode | Country |
 * ... | Subtotal | Tax | Shipping | Discount | Total
 *
 * Returns Sale[] normalized objects (same shape as parse-store).
 */
export async function parseTradeshowExcel(filePath) {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(filePath);

  const sheet = workbook.worksheets[0];
  if (!sheet) throw new Error('No worksheet found in Trade Show POS Excel');
...
function resolveColumns(headers) {
  return {
    transactionNo: findCol(headers, ['transaction no', 'transaction_no', 'transactionno', 'trans no', 'order no']),
